...
def generate_orchestrator_snippet(model, user_description: str) -> str:
    """
    Generates a single component snippet without file I/O side effects.
    This is designed to be imported and used by another script.
    """
    prompt_for_llm = (
        "You are an expert Framework Architect creating reusable text snippets for a larger system. "
        "Based on the following description, write a clear, professional, and concise text snippet. "
        "The output must be ONLY the raw text of the snippet itself, with no conversational text or markdown formatting.\n\n"
        f"**Component Description:** '{user_description}'"
    )
    try:
        response = model.generate_content(prompt_for_llm)
        return response.text
    except Exception as e:
        logging.error(f"Error in generate_orchestrator_snippet: {str(e)}")
        # Re-raise the exception so the calling script can handle it.
        raise
